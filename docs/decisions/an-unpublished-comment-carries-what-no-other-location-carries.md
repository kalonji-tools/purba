...
|---|---|
| pull request | the discussion between the implementer, the co-author and the reviewer |
| commit message | the short resolution, with the issue number for the deeper dive |
| decision record | what the project decided, as current state |
| published comment | the use and existence of an exposed artifact, and of a heavily used private one |
| unpublished comment | undecided until now |

The first four have a register.
The fifth does not, so a writer with something to say puts it in all five.
...
**The order of claim.**
The pull request takes the discussion.
The commit takes the resolution, and the issue number in its subject takes the deeper dive.
The decision record takes what the project decided.
The published comment takes the use and existence of an artifact.
